...
from pyspark.sql.functions import col, when, lit, avg, stddev, expr, abs as spark_abs
...
class ETLPipeline:
    ...
    def extract(self, input_path: str) -> None:
        ...
        def detect_column_type(df: DataFrame, col_name: str) -> str:
            ...
            type_counts = {"int": 0, "double": 0, "boolean": 0, "date": 0}
        ...
        for col_name in df.columns:
            ...
            if detected_type != "string":
                df = df.withColumn(
                    col_name,
                    when(
                        col(col_name).cast(detected_type).isNotNull(),
    ...
    def _convert_credit_history_to_months(self, df: DataFrame) -> DataFrame:
        return df.withColumn(
            ...
            when(
                col("Credit_History_Age").contains("Years and"),
                expr(
                    "cast(regexp_extract(Credit_History_Age, '(\\d+) Years and', 1) as int) * 12 + "
                    + "cast(regexp_extract(Credit_History_Age, 'and (\\d+) Months', 1) as int)"
                ),
            )
            .when(
                col("Credit_History_Age").contains("Years"),
                expr(
                    "cast(regexp_extract(Credit_History_Age, '(\\d+) Years', 1) as int) * 12"
                ),
            )
            .when(
                col("Credit_History_Age").contains("Months"),
                expr(
                    "cast(regexp_extract(Credit_History_Age, '(\\d+) Months', 1) as int)"
                ),
            )
            .otherwise(None),
        )
    ...
    def _remove_outliers(
        self, df: DataFrame, numeric_cols: List[str], threshold: int = 3
    ) -> DataFrame:
        for col_name in numeric_cols:
            mean = df.select(avg(col_name)).collect()[0][0]
            std = df.select(stddev(col_name)).collect()[0][0]
            df = df.filter(spark_abs((col(col_name) - mean) / std) < threshold)
        return df
    ...
    def _process_silver_layer(self) -> DataFrame:
        ...
        for col_name in numeric_cols:
            try:
                ...
                logger.warning(
                    f"Error while processing numeric column {col_name}: {str(e)}"
                )
                df = df.fillna(0, subset=[col_name])
        ...
        for col_name in categorical_cols:
            try:
                ...
                logger.warning(
                    f"Error while processing categorical column {col_name}: {str(e)}"
                )
                df = df.fillna("unknown", subset=[col_name])
    ...
    def _process_gold_layer(self, silver_df: DataFrame) -> DataFrame:
        ...
        for col_name in numeric_cols:
            try:
                percentiles = silver_df.approxQuantile(
                    col_name, [0.25, 0.5, 0.75], 0.01
                )
                if percentiles and len(percentiles) == 3:
                    stats_df = (
                        stats_df.withColumn(f"{col_name}_q1", lit(percentiles[0]))
                        .withColumn(f"{col_name}_median", lit(percentiles[1]))
                        .withColumn(f"{col_name}_q3", lit(percentiles[2]))
                    )
            except Exception as e:
                logger.warning(
                    f"Could not calculate percentiles for {col_name}: {str(e)}"
                )
...
def create_etl_pipeline(
    spark: SparkSession, root_dir: str, do_repartition: bool = False
):
